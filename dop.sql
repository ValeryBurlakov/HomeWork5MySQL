...
WHERE
    TIMESTAMPDIFF(YEAR, `profiles`.birthday, NOW()) <= 20;
...
SELECT 
  ...
  DENSE_RANK() OVER (ORDER BY COUNT(messages.id) DESC) AS message_rank
FROM
  users
...
-- когда указал время в часах, в половине столбцов было 838:59:59, а в днях ниже сделал
SELECT 
  messages.id,
  messages.created_at,
  LEAD(messages.created_at) OVER (ORDER BY messages.created_at) AS next_created_at,
  TIMESTAMPDIFF(SECOND, messages.created_at, LEAD(messages.created_at) OVER (ORDER BY messages.created_at)) AS time_diff_in_seconds
FROM
  messages
ORDER BY
  messages.created_at;
  
-- по умолчанию идет сортировка ASC, можно писать её, а можно не писать
-- SELECT 
--   messages.id,
--   messages.created_at,
--   LEAD(messages.created_at) OVER (ORDER BY messages.created_at ASC) AS next_created_at,
--   TIMESTAMPDIFF(SECOND, messages.created_at, LEAD(messages.created_at) OVER (ORDER BY messages.created_at ASC)) AS time_diff_in_seconds
-- FROM
--   messages
-- ORDER BY
--   messages.created_at ASC;

-- то же, что и выше, только добавил столбец дней если прошло меньше 24 часов, будет 0
SELECT   messages.id,
  messages.created_at,  LEAD(messages.created_at) OVER (ORDER BY messages.created_at) AS next_created_at,
  TIMESTAMPDIFF(SECOND, messages.created_at, LEAD(messages.created_at) OVER (ORDER BY messages.created_at)) AS time_diff_in_seconds,
  TIMESTAMPDIFF(DAY, messages.created_at, LEAD(messages.created_at) OVER (ORDER BY messages.created_at)) AS time_diff_in_days
FROM
messages
ORDER BY
messages.created_at;